<template>
  <scroll-view class="form-page" scroll-y>
    <view class="form-card">
      ...
      <view class="field"><text class="label">路线名称</text><input v-model="form.name" class="input" placeholder="例如：川西小环线三日骑行" /></view>
      <view class="field"><text class="label">路线描述</text><textarea v-model="form.description" class="textarea" placeholder="写下路况、补给点与风景亮点" /></view>
      <view class="grid">
        <view class="field"><text class="label">预计距离(km)</text><input v-model="form.distance" class="input" type="number" /></view>
        <view class="field"><text class="label">预计时长(h)</text><input v-model="form.duration" class="input" type="number" /></view>
      </view>
      <view class="field"><text class="label">路线难度</text><picker :range="difficultyOptions" @change="onDifficultyChange"><view class="picker">{{ difficultyOptions[selectedDifficulty] }}</view></picker></view>
      <view class="switch-row"><text>公开给社区</text><switch :checked="form.isPublic" color="#FF6B35" @change="form.isPublic = $event.detail.value" /></view>
...
const form = reactive({ name: '', description: '', distance: '180', duration: '5', isPublic: true })
...
.input,.picker,.textarea { width: 100%; box-sizing: border-box; background: #f7f8fa; border-radius: 18rpx; padding: 22rpx 20rpx; font-size: 28rpx; }
